...
export default async function handler(
  ...
  res: NextApiResponse<Data>
    ...
    const bigIntIdentityNullifier = BigInt(identityNullifier)
    let splitIdentityNullifier;
    if (typeof(bigIntIdentityNullifier) == "bigint") {
        splitIdentityNullifier = splitPrivateKey(BigInt(identityNullifier));
    } else {
        res.status(200).json({
            name: "Not a valid identity nullifier", proofForTx: [], nullifierHash: ""
        })
    }
